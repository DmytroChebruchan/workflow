...
async def test_check_node_type_existence_in_workflow():
    ...
    with patch(
        "api.nodes.validator.get_workflow_by_id"
    ) as mock_get_workflow_by_id, patch(
        "api.nodes.validator.ensure_unique_node_type"
    ) as mock_ensure_unique_node_type:
